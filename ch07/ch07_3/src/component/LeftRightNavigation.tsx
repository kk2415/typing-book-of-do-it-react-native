import React, { FC, ForwardRefRenderFunction, MutableRefObject, ReactNode, useImperativeHandle, useState, forwardRef } from "react";
...
const _LeftRightNavigation: ForwardRefRenderFunction<LeftRightNavigationMethods, LeftRightNavigationProps> = (
	...
	const panResponder = usePanResponder({
		onPanResponderMove(e, s) {
			...
			if (Math.abs(dy) < 30) { //수직 스크롤이 거의 없을 때만 적용
				if (dx > distance) { //왼쪽에서 오른쪽 제스처 길이
					onLeftToRight && onLeftToRight()
				} else if (dx < -distance) { //오른쪽에서 왼쪽 제스처 길이
...
const styles = StyleSheet.create({
	view: {width: '100%', flex: 1, backgroundColor: Platform.select({ios: undefined, android: 'transparent'})}
})
